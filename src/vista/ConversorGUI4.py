...
class ConversorApp:
    def __init__(self, root):
        ...
        # Fondo degradado
        self.background = tk.Canvas(root, width=450, height=350)
        self.background.pack(fill="both", expand=True)
        self.background.create_rectangle(0, 0, 450, 350, outline="", fill="#d1e8e2")
        self.background.create_rectangle(0, 200, 450, 350, outline="", fill="#89c9b8")
        ...
        style.configure("Custom.TButton", background="#57a773", foreground="white", font=("Helvetica", 10, "bold"), padding=8)
        style.map("Custom.TButton", background=[("active", "#45a049")])

        # Marco central con bordes y fondo blanco
        self.frame = ttk.Frame(root, padding="20 20 20 20", style="Custom.TFrame")
        self.frame.place(relx=0.5, rely=0.5, anchor="center")

        # Etiqueta de título
        self.title_label = ttk.Label(self.frame, text="Conversor de Monedas", font=("Helvetica", 16, "bold"), background="#ffffff", foreground="#4b6584")
        ...
        self.origen_combo = ttk.Combobox(self.frame, values=['USD', 'EUR', 'JPY'], width=18, font=("Helvetica", 12))
        ...
        self.destino_combo = ttk.Combobox(self.frame, values=['USD', 'EUR', 'JPY'], width=18, font=("Helvetica", 12))
        self.destino_combo.grid(row=3, column=1, pady=5)
        self.destino_combo.current(1)

        # Botón de conversión con estilo mejorado
        self.convertir_btn = ttk.Button(self.frame, text="Convertir", command=self.convertir, style="Custom.TButton", cursor="hand2")
        self.convertir_btn.grid(row=4, column=0, columnspan=2, pady=(15, 10))

        # Etiqueta de resultado con un estilo más llamativo
        self.resultado_label = ttk.Label(self.frame, text="Resultado: ", font=("Helvetica", 12, "italic"), style="Custom.TLabel", background="#ffffff", foreground="#333333")
